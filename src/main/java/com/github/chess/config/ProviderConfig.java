 ...
 * openai 这条路没有这个概念，摆到棋手配置里只会让 gpt 的棋手都杵着一个对自己无效的开关。
 * 想让同一个模型开着思考和关着思考各下一盘，就配两条指向同一地址、只有这个开关不同的连接。
 *
 * @param type     接口格式，anthropic 或 openai。同一个自定义地址换个格式即可对接另一套协议，留空按 anthropic 处理
 * @param baseUrl  服务地址，留空表示使用该类型的默认地址
 * @param apiKey   访问密钥，留空时该连接下的棋手会直接走启发式兜底
 * @param thinking 是否开启扩展思考，只对 anthropic 生效，留空按关闭处理。openai 配了会被忽略
 * @author yaoyuquan
 */
public record ProviderConfig(String type, String baseUrl, String apiKey, Boolean thinking) {
    ...
    public static final List<String> SUPPORTED_TYPES = List.of(TYPE_ANTHROPIC, TYPE_OPENAI);
